...
function UserPostUpdateSettings({ setPostSetting, postSettings, currentMentionList }) {
  ...
  useEffect(() => {
    if (mentionSearchText) {
      const fetchSearchedUser = async () => {
        const response = await searchService.searchByUsername(mentionSearchText);
    ...
    <div>
      <div className="fixed h-fit sm:left-24 top-0 sm:-mx-8 z-50">
        ...
        <div
          ...
          } bg-white border pt-10 shadow-xl`}
        >
          <div>
            <div onClick={() => setSelectMentions(true)} className="flex gap-5 py-3 text-lg font-medium items-center">
              <svg
                className="h-6 w-6"
                viewBox="0 0 24 24"
                fill="none"
              ...
              <div className="mt-1 mb-2 flex gap-2 text-sm flex-wrap">
                {mentionList?.map((user) => (
                  ...
                  >
                    ...
                    <svg
                      onClick={() => setMentionList(mentionList.filter((u) => u._id !== user._id))}
              ...
              <div className="px-4 ">
                {mentionSearchList.map((user) => (
                  <div
                    key={user._id}
                    onClick={() => {
                      if (!mentionList.some((listUser) => listUser._id === user._id)) {
           ...
           {selectMentions && <button 
            onClick={()=>{
              updatePost(postSettings._id,
                {...postSettings,mentions:mentionList}
              )
              setPostSetting(null)
              setSelectMentions(false)
            }
          }
            className="w-full bg-blue-500 mb-6 font-medium py-2.5 rounded-lg text-white">Update</button>}
          </div>
          <div
            onClick={() => {
