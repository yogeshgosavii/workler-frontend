...
const LocationInput = ({
  ...
  const createError = () => {
    if(visited){
      ...
      } else if (isRequired && !locations.some(loc => loc.id === selectedLocation?.id)) {
    ...
    <div className="relative">
      <div className="relative flex items-center">
        <input
          ...
          className={`flex-1 block px-3 py-3 font-normal pr-[75px] bg-white rounded-sm border appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer ${className}`}
          style={{
      ...
      {inputFocus && !loading && locations.length > 0 && (
